...
# Vision-only reference for the Wave 1/2 matrix.
# Recipe matches the multimodal runs: two-GPU DDP, EMA, decoder DS2, and the
# same optimizer-update/effective-batch schedule. No LLM or text cache is used.
...
CHECKPOINT_BASE="${CHECKPOINT_BASE:-/mnt/nas125/forGPU2/lhyunki/llmseg/experiments/fudata_final/llama/hybrid_wave0_vision_ema_ds2}"
LOG_ROOT="${LOG_ROOT:-${PROJECT_DIR}/train_logs/hybrid_wave0_vision_ema_ds2_2gpu_$(date +%Y%m%d_%H%M%S)}"
...
if [ "$SMOKE_TEST" = "1" ]; then
    ...
    OVERWRITE_TRAIN=1
    CHECKPOINT_BASE="${CHECKPOINT_BASE_SMOKE:-${PROJECT_DIR}/_debug_ckpt/hybrid_wave0_vision_ema_ds2}"
    LOG_ROOT="${LOG_ROOT_SMOKE:-${PROJECT_DIR}/train_logs/hybrid_wave0_vision_ema_ds2_2gpu_smoke_$(date +%Y%m%d_%H%M%S)}"
    export CUDA_LAUNCH_BLOCKING="${CUDA_LAUNCH_BLOCKING:-1}"
fi

mkdir -p "$LOG_ROOT" "$CHECKPOINT_BASE"

echo "[PLAN] Vision-only reference on GPU pair $GPU_PAIR"
...
ONLY_EXPERIMENTS=vision_only \
SOFT_PROMPT_MODE=disabled TEXT_FEATURE_CACHE="" DICOM_PROMPT_MODE=none \
EXPERIMENT_NAME_SUFFIX="${EXPERIMENT_NAME_SUFFIX:-_wave0_vision_ema_ds2}" \
CHECKPOINT_BASE="$CHECKPOINT_BASE" LOG_ROOT="$LOG_ROOT" \
BASE_PORT="${BASE_PORT:-30300}" STREAM_LOGS="${STREAM_LOGS:-1}" \
